...
import com.fighter.quickstop.lib.Sliding.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
	...
	private int mTouchMode = Sliding.TOUCHMODE_MARGIN;
	...
	public void setMode(int mode) {
		if (mode == Sliding.LEFT || mode == Sliding.RIGHT) {
			if (mContent != null)
	...
	public int getMenuPage(int page) {
		...
		if (mMode == Sliding.LEFT && page > 1) {
			return 0;
		} else if (mMode == Sliding.RIGHT && page < 1) {
			return 2;
		} else {
			return page;
		}
	...
	public int getAbsLeftBound(View content) {
		if (mMode == Sliding.LEFT || mMode == Sliding.LEFT_RIGHT) {
			return content.getLeft() - getBehindWidth();
		} else if (mMode == Sliding.RIGHT) {
			return content.getLeft();
		}
	...
	public int getAbsRightBound(View content) {
		if (mMode == Sliding.LEFT) {
			return content.getLeft();
		} else if (mMode == Sliding.RIGHT || mMode == Sliding.LEFT_RIGHT) {
			return content.getLeft() + getBehindWidth();
		}
		return 0;
	}
	...
	public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
		switch (mTouchMode) {
		case Sliding.TOUCHMODE_FULLSCREEN:
			return true;
		case Sliding.TOUCHMODE_MARGIN:
	...
	public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
		if (mMode == Sliding.LEFT || (mMode == Sliding.LEFT_RIGHT && currPage == 0)) {
			return x >= content.getLeft();
		} else if (mMode == Sliding.RIGHT || (mMode == Sliding.LEFT_RIGHT && currPage == 2)) {
