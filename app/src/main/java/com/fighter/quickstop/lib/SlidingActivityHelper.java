...
public class SlidingActivityHelper {
	...
	public void onCreate(Bundle savedInstanceState) {
		mSliding = (Sliding) LayoutInflater.from(mActivity).inflate(R.layout.slidingmenumain,null);
	...
	public void onPostCreate(Bundle savedInstanceState) {
		...
		mSliding.attachToActivity(mActivity,
				mEnableSlide ? Sliding.SLIDING_WINDOW : Sliding.SLIDING_CONTENT);
		...
		new Handler().post(new Runnable() {
			public void run() {
				if (open) {
					if (secondary) {
						mSliding.showSecondaryMenu(false);
					} else {
						mSliding.showMenu(false);
					}
				} else {
					mSliding.showContent(false);
				}
			}
		});
	}
	...
	public void onSaveInstanceState(Bundle outState) {
		outState.putBoolean("SlidingActivityHelper.open", mSliding.isMenuShowing());
		outState.putBoolean("SlidingActivityHelper.secondary", mSliding.isSecondaryMenuShowing());
	}
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && mSliding.isMenuShowing()) {
			showContent();
			return true;
		}
		return false;
